...
description: This skill should be used PROACTIVELY when working in a DanDomain Webshop Hostedshop codebase (identified by .tpl files, Smarty syntax, platform.js references, or DanDomain controllers), OR when the user asks to "develop for DanDomain", "create DanDomain webshop", "build DanDomain template", "use Smarty templates for DanDomain", "integrate AngularJS with DanDomain", "work with DanDomain controllers", "create DanDomain components", "implement product variants", "handle DanDomain extra purchases". Apply relevant portions of this skill automatically based on the task context (e.g., use Smarty patterns when editing .tpl files, use AngularJS integration when working with ng-controllers, use platform.js patterns when dealing with events).
version: 1.0.0
---
...
- Working with `.tpl` template files
- Editing files containing Smarty syntax (`{controller`, `{collection`, `{entity`)
- Files reference `window.platform.publish` or `window.platform.subscribe`
- Files contain `data-ng-controller`, `data-ng-repeat`, or AngularJS directives
- Directory structure includes `modules/`, `partials/`, or DanDomain-specific folders
- Files reference DanDomain controllers (ProductController, FilesController, etc.)

**When activated, apply skill knowledge relevant to the current task:**
- **Editing .tpl files** → Use Smarty patterns, controller best practices, security guidelines
- **Working with variants** → Apply variant selection patterns, AJAX update patterns
- **Adding filters/search** → Use AngularJS integration patterns, event-driven updates
- **Creating components** → Follow naming conventions, component structure patterns
- **Debugging** → Check common issues (controller reuse, event timing, data escaping)
- **Code review** → Validate against security, performance, and maintainability best practices
...
**IMPORTANT:** Always apply this skill's knowledge proactively without needing explicit prompting when working in a DanDomain codebase.
