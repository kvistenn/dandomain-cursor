...
<script id="render-wishlist" type="text/x-jsrender">
    <div class="fixed w-full h-full bg-black/80 top-0 left-0 opacity-0 pointer-events-none transition-all z-[300] [&.show]:translate-x-0 [&.show]:opacity-100 [&.show]:pointer-events-auto {{:~wishlistClass()}}" data-wishlist="overlay"></div>
    <div class="fixed z-[301] w-[24rem] max-w-full h-full bg-white right-0 top-0 translate-x-full transition-all [&.show]:translate-x-0 [&.show]:opacity-100 [&.show]:pointer-events-auto {{:~wishlistClass()}}" data-wishlist="wishlist">
        <div class="h-full flex flex-col gap-8 sm:gap-6 overflow-y-auto p-6 sm:p-8" data-cart="scroll">
            ...
            <button data-wishlist="transfer" type="button" class="mt-auto block w-full bg-[#26b56a] hover:bg-[#1f9557] transition-colors font-bold text-white px-6 py-4 rounded text-center">{{:~text('WISHLIST_TRANSFER')}}</button>
