...
{function logoImage source='' style=false}{strip}
    <a href="{$general.domain}" style="text-decoration:none;" title="{$contactdata.name|escape:'html'}"><img style="height:auto;max-width:200px;{if $style}{$style}{/if}" src="{if $request->isPrint and $request->isPDF}{$source|pdfPath}{else}{$source|solutionPath}{/if}" alt="{$contactdata.name|escape:'html'}" /></a>
{/strip}{/function}
