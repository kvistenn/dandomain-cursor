;(function($, platform, exports) {
	...
	exports.platform.checkout =
	{
		...
		binds: function() {
			...
			$('#deliveryActive').on('change', function() {
				var checked = $(this).is(':checked');
				if(!checked) {
					$('[data-addresses="select"]', self.wrapper).val('');
					$('[name="deliveryFirstname"]').val("");
					$('[name="deliveryLastname"]').val("");
					$('[name="deliveryCompany"]').val("");
					$('[name="deliveryAddress"]').val("");
					$('[name="deliveryZipcode"]').val("");
					$('[name="deliveryCity"]').val("");
					$('[name="deliveryCountry"]').val("DK");
					$('[name="deliveryPhone"]').val("");
				}
			});

			$('[data-addresses="select"]', self.wrapper).on('change', function() {
				var $select = $(this);
				var data = JSON.parse($select.val());

				if(!$('#deliveryActive').prop('checked')) {
					$('#deliveryActive').prop('checked', true);
				...
				$('[name="deliveryFirstname"]').val(platform.user.Firstname);
				$('[name="deliveryLastname"]').val(platform.user.Lastname);
				$('[name="deliveryCompany"]').val(data.name || platform.user.Company);
				$('[name="deliveryAddress"]').val(data.address || platform.user.Address);
				$('[name="deliveryZipcode"]').val(data.zipCode || platform.user.Zipcode);
				$('[name="deliveryCity"]').val(data.city || platform.user.City);
				$('[name="deliveryCountry"]').val(country);
				$('[name="deliveryPhone"]').val(platform.user.Phone);
				$('[name="customCustomerNotice"]').val(data.attention || "-");
				$('[name="customCustomerNotice"]').trigger('keyup');

				if($('[name="customCustomerNotice"]').val() != "-") {
					$('[name="customCustomerNotice"]').show();
				} else {
					$('[name="customCustomerNotice"]').hide();
				}
			});

			$('[name="customCustomerNotice"]', self.wrapper).on('keyup', function() {
				var $this = $(this);
				$('[name="customerNotice"]', self.wrapper).val($this.val());
			});

			$('[name="customerNotice"]', self.wrapper).on('keyup', function() {
				var $this = $(this);
				$('[name="customCustomerNotice"]', self.wrapper).val($this.val());
			});
		},
		...
		generateOverview: function (data) {
			...
			console.log("Generating overview with data:", data);
			console.log("Generating overview with data:", platform.checkout.STEP1.relevantFieldDatas());
		...
		STEP1: {
			...
			binds: function () {
				...
				if (customerTypeInput.length) {
					...
					setTimeout(function () {
						var initialType = customerTypeInput.filter(':checked').val() || customerTypeInput.first().val();
						toggleCustomerType(initialType);
					}, 100);
			...
			actions: function () {
				...
				// Check the customer type checkbox if active
				if($('[name="customer-type"]').length && !platform.user) {
					var languageIso = (platform && platform.general && platform.general.languageIso) ? platform.general.languageIso : null;
					var forceCompany = (languageIso === 'SE26' || languageIso === 'DE26');
					setTimeout(function() {
						if (forceCompany) $('[name="customer-type"][value="company"]').trigger('click');
					}, 100);
				}
			},
	        ...
	        loaded: function(data) {
				...
				var $customerTypeInputs = $('input[name=customer-type]', this.wrapper);
				if ($customerTypeInputs.length) {
					// Show customer-type input and enable dynamic fields
					if ($customerTypeInputs.length > 1) {
