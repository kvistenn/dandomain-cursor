...
{if $field}
    ...
    {if $products and $products->getActualSize() gt 0}
        ...
        {foreach $products->getData() as $k => $product}
            ...
            {$specialPrice = false}
            {if $user}
                {foreach $priceLines->getData() as $priceLine}
                    {if $priceLine->UserType == 'group' && $priceLine->UserId == $user->CategoryId && $priceLine->Currency == $general.currencyIso}
                        {$specialPrice = true}
                    {/if}

                    {if $priceLine->UserType == 'user' && $priceLine->UserId == $user->Id && $priceLine->Currency == $general.currencyIso}
                        {$specialPrice = true}
                    {/if}
                {/foreach}
            {/if}
            {$product['SpecialPrice'] = $specialPrice}
            {$product['PriceLines'] = $priceLines->getData()}
