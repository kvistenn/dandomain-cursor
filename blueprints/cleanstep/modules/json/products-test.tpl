...
{$variantId = null}
{if isset($request.args[1])}
    {$variantId = $request.args.1}
{elseif isset($request.input['variantid'])}
...
{if $field}
    ...
    {if $field eq 'id'}
        {if $value}
            {$value = explode(',', $value)}
            {if $variantId}
                {collection assign=products controller=$variantController productId=$value variantId=$variantId}
            {else}
    ...
    {if $products and $products->getActualSize() gt 0}
        ...
        {foreach $products->getData() as $k => $product}
            ...
            {if $customLabels->getActualSize() gt 0}
                {foreach $customLabels->getData() as $label}
                    {if $label->DataId eq 1} {* Ecolabel *}
                        {$labels[] = ['Title' => 'Ecolabel', 'Image' => "{$template.path}/assets/images/labels/ecolabel.webp"]}
                    {else if $label->DataId eq 2} {* FSC *}
                        {$labels[] = ['Title' => 'FSC', 'Image' => "{$template.path}/assets/images/labels/fsc.webp"]}
                    {else if $label->DataId eq 3} {* Svanemærket *}
                        {$labels[] = ['Title' => 'Svanemærket', 'Image' => "{$template.path}/assets/images/labels/svanemaerket.webp"]}
                    {else if $label->DataId eq 4} {* Plant Based *}
                        {$labels[] = ['Title' => 'Plant Based', 'Image' => "{$template.path}/assets/images/labels/plantbased.webp"]}
                    {else if $label->DataId eq 5} {* Cradle to Cradle *}
                        {$labels[] = ['Title' => 'Cradle to Cradle', 'Image' => "{$template.path}/assets/images/labels/cradle-to-cradle.webp"]}
                    {/if}
                {/foreach}
            {/if}
            ...
            {* Kolli *}
            {$kolliAmount = null}
            {entity assign=kolli controller=productCustomData typeId=2 productId=$product.Id}
            {if $kolli}
                {$kolliAmount = $kolli->Title}
            {/if}
            {$product['Kolli'] = $kolliAmount}
...
{$output = ['amount' => $amount, 'productIds' => $productIds, 'products' => $output, 'filterMap' => $filterMap, 'variantId' => $variantId, 'productvariants' => $products->getData()]}
