;(function ($, eksport, undefined) {
    ...
    var controller = {
        init : function (el) {
            ...
            self.$variantInput         = $('[data-controller="variant"]');
        ...
        handleSelect : function(event, data) {
            ...
            if (item) {
                ...
                self.$variantInput.val(item.Id);
                
                self.product.updateItemNumber(item);
                self.product.updateStock(item);
                self.product.updateDescriptions(item);
                self.product.updatePrices(item);
