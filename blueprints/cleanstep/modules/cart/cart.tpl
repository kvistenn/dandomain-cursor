            ...
            <div class="pt-5 flex-col md:flex-row flex justify-between gap-5">
                <div>
                    <div class="justify-between sm:justify-start flex flex-wrap sm:flex-nowrap gap-3 lg:gap-5">
                        <a href="{$page.lastPath}" class="text-xs block w-fit lg:text-sm bg-black text-white px-4 lg:px-6 py-2 lg:py-3 rounded text-center">{$text.CART_BUY_MORE}</a>
                ...
                <div class="w-full md:w-80 lg:w-[500px] max-w-full">
                    ...
                    <div class="flex">
                        <a class="p-4 text-white bg-black transition duration-200 rounded sm:rounded-l-none uppercase text-sm tracking-wider font-bold flex-auto justify-center text-center" href="/{$cartLink}/">{$text.CHECKOUT_MENU}</a>
                    </div>
                </div>
            </div>

            <div class="mt-6 lg:mt-12">
                <div id="hr-recom-k68875a619c99e26ba24b3106"></div>
            </div>
        ...
        {if ($access.discountCode or $access.giftCard) and $settings.module_shop_discount_codes and !isset($cartList.CartCouponLine)}
            <div id="coupon" style="display: none; width: 600px; max-width: 100%" tabindex="-1" role="dialog" aria-hidden="true">
                <form method="post" action="/actions/cart/couponAdd" class="flex flex-col gap-2">
                    ...
                    <button type="submit" class="block w-fit text-sm bg-black text-white px-8 py-3 rounded text-center mt-3">{$text.SHOP_DISCOUNT_CODE_CONFIRM}</button>
                </form>
            </div>
        {/if}
