        ...
        function ($scope, $q, $rootScope, $attrs, $filter, $location, $api, $timeout, LinkService, FilterService) {
            ...
            function updateProductlist(searchFilterSelected) {
                ...
                return $api.productlist.query(attributes, function(data) {
                    ...
                    if (attributes.filter.stock) {
                        setTimeout(function() {
                            $('.product[data-instock="false"]').parent().addClass('hidden');
                        });
                    } else {
                        setTimeout(function() {
                            $('.product[data-instock="false"]').parent().removeClass('hidden');
                        });
                    }
                    setTimeout(function() {
                        if ($('.product[data-instock="true"]').length <= 0) {
                            $('#filter-showstock').attr('disabled', 'disabled');  
                            $('#filter-showstock+span').addClass('pointer-events-none opacity-50');   
                        } else {
                            $('#filter-showstock').removeAttr('disabled');
                            $('#filter-showstock+span').removeClass('pointer-events-none opacity-50');  
                        }
                    });
