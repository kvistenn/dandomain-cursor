...
{if $request.method eq 'GET'}
	...
	{if $product}
		...
		{else}
            ...
            {if !empty($request.input.variantId)}
                {$options['variantId'] = $request.input.variantId}
            {/if}
			...
			{foreach $variantData->getData() as $variant}
				...
				{entity assign=variantDatasFromId controller=productVariantData productId=$product->Id variantId=$variant->Id}
				{$variant['remoteStock'] = $variantDatasFromId->Sorting}
				...
				{else}
					{if !$variant.Soldout}
						...
						{elseif $variant['remoteStock'] gt 0}
						{$stocktext = $text.PRODUCT_REMOTE_INSTOCK}
						{else}
						{$stocktext = $text.PRODUCT_OUTOFSTOCK_BOTH}
					{/if}
				{/if}
