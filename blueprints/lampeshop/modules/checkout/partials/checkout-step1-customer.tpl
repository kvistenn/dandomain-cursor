...
<div class="flex flex-col">
    ...
    {* <div class="py-7 border-b border-gray-300 relative">
        <span class="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-2 bg-white inline-block text-sm">{$text.CHECKOUT_QUICK}</span>
        <div class="flex gap-3">
            <button class="flex-1 px-4 h-11 rounded bg-[#5173ff] text-white text-sm">
                <img src="{$template.path}/assets/images/payment/logo-mobilepay.svg" alt="MobilePay" class="w-auto h-5 block mx-auto">
            </button>
            <button class="flex-1 px-4 h-11 rounded bg-gray-100 text-white text-sm">
                <img src="{$template.path}/assets/images/payment/logo-gpay.svg" alt="Google Pay" class="w-auto h-5 block mx-auto">
            </button>
            <button class="flex-1 px-4 h-11 rounded bg-black text-white text-sm">
                <img src="{$template.path}/assets/images/payment/logo-applepay.svg" alt="Apple Pay" class="w-auto h-5 block mx-auto">
            </button>
        </div>
        <span class="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 px-2 bg-white inline-block text-sm">{$text.POS_GENERAL_OR}</span>
    </div> *}
    ...
    <div class="py-5">
        ...
        {if $access.user && $settings.news_signup}
            {* Newsletter signup *}
            <input type="checkbox" name="newsletter" id="newsletter" class="hidden" {if $returnPostData.newsletter}checked{/if} checked />
            <label class="flex gap-3 my-4">
                <input type="checkbox" name="newsletterCustom" id="newsletterCustom" class="w-4 h-4" {if $returnPostData.newsletter}checked{/if} />
                <span class="text-xs md:text-sm text-gray-500">{$Text.USER_NEWSLETTER_NOT}</span>
            </label>
        {/if}
        ...
        <div class="m-checkout-customer-type py-5" style="display:none">
            ...
            <div class="flex flex-col gap-4">
                <div class="flex bg-white border border-gray-300 rounded w-full overflow-hidden">
                    <label class="flex-1 flex items-center justify-center gap-3 text-sm p-3 border-r border-gray-300 cursor-pointer bg-black text-white">
                    ...
                    {if $settings.shop_customer_company}
                        <label class="flex-1 flex items-center justify-center gap-3 text-sm p-3 border-r border-gray-300 cursor-pointer text-gray-500">
                            <input type="radio" name="customer-type" class="hidden md:block w-4 h-4" value="company">
                            <span class="text-xs md:text-sm">{$text.CUSTOMER_TYPE_COMPANY}</span>
                        </label>
                    {/if}
                    {if $settings.shop_customer_institution}
                        <label class="flex-1 flex items-center justify-center gap-3 text-sm p-3 cursor-pointer text-gray-500">
            ...
            {if $settings.shop_customer_company}
                <div class="customer-type-dynamic-fields" data-active-on-customer-type="company" style="display:none">
                    <div class="flex flex-col gap-4">
                        ...
                        <div>
                            <input type="text" name="vatNumber" autocomplete="disable" value="{$returnPostData.vatNumber}" id="vatNumber" class="bg-white border border-gray-300 rounded w-full px-3 h-10 md:h-12 text-xs md:text-sm" placeholder="{$Text.VAT_NR}" data-is-required disabled data-active-on-customer-type="company" style="display: none;">
                            <span class="error text-red-500 text-xs block mt-2 empty:hidden"></span>
                        </div>
                        <div>
