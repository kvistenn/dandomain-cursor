...
<div id="repay" itemscope itemtype="http://schema.org/CheckoutPage">
	<div class="modules m-repay container mx-auto py-6 sm:py-10 md:py-14">
		<header class="m-repay-header page-title mb-2">
			<h1 class="m-repay-headline text-lg md:text-xl lg:text-2xl font-bold">{$text.CHECKOUT_HEADLINE}</h1>
		</header>
					...
					{foreach $paymentMethods->getData() as $method}
						<li class="paymentMethodContainer">
							<fieldset class="form-group small">
								<div class="input-group flex gap-4 items-center border-t border-gray-300 py-2">
									...
									<div class="form-label input-group-main text-xs text-gray-500 font-bold">
										<label for="payment_type_{$method->Id}{if $method->Klarna}_klarna_{$method->Klarna.group}{/if}">
											<span class="paymentMethodTitle">
												{if $method->Klarna}
													<span class="l-payment-method-title-logo klarnaLogo block">
											...
											{if $method->Description}
												<span class="paymentMethodDescription block">{$method->Description}</span>
											{/if}
											{if $method->PriceFixed}
												<span class="block">{$method->PriceFixed}</span>
											{/if}
											{if $method->PricePercentage}
												<span class="block">{$method->PricePercentage} <span class="m-percent">%</span></span>
											{/if}
										</label>

										{if $method->Id eq 6}
											<span class="paymentMethodPaypalIcons flex flex-wrap gap-1">
										...
										{elseif $method->Klarna}
											<div class="m-klarna-container showOnClick" style="display:none;">
														...
														{foreach $method->Klarna["paymentMethods"] as $klarnaMethod}
															<li>
																<fieldset class="form-group">
																    <div class="input-group flex gap-4 items-center border-t border-gray-300 py-2" style="width:100%;">
																        <span class="input-group-addon">
																        ...
																        <div class="form-label input-group-main text-xs text-gray-500 font-bold">
																			...
																			<div id="klarna_details_{$klarnaMethod.group}_{$klarnaMethod@index}" class="klarnaShowOnClick" style="display:none;">
																				...
																				<p class="klarna_example">
																					<span class="m-klarna-exampletext block">{$klarnaMethod.exampleText}</span>
																					<a class="m-klarna-terms-button block fancybox fancybox.iframe" href="{$klarnaMethod.terms}">{$text.PRODUCT_CATALOG_READ_MORE}</a>
																				</p>
																				{/if}
																			</div>
																        </div>
												...
												<fieldset class="form-group m-klarna-label">
													<label class="form-label text-xs text-gray-500 font-bold" for="cpr_{$method->Klarna.group}">
											...
											<li>
												<fieldset class="form-group small">
													<div class="input-group flex gap-4 items-center border-t border-gray-300 py-2">
														...
														<label for="pbs_type_{$onlineMethod->Id}" class="form-label input-group-main text-xs text-gray-500 font-bold">
															<span class="block">{$onlineMethod->Title}</span>
															{if $onlineMethod->PriceFixed}
																<span class="block">{$onlineMethod->PriceFixed}</span>
															{/if}
															{if $onlineMethod->PricePercentage}
																<span class="block">{$onlineMethod->PricePercentage} %</span>
															{/if}
