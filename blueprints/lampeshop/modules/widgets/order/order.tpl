...
<div class="w-order">
	<div class="w-order-wrap w-userinformation-wrap">
	...
	<div class="w-order-wrap w-order-invoice-wrap">
...
<div class="m-my-orders w-order-details">

		<div class="w-order-headline-wrap">
			<div class="w-order-cart-product">
				<b>{$text.CART_PRODUCTS}</b>
			</div>
			<div class="w-order-cart-note">
				<b>{$text.CART_NOTE}</b>
			</div>
			<div class="w-order-cart-count">
				<b>{$text.CART_COUNT}</b>
			</div>
			<div class="cart-total-value w-order-cart-total-value">
				<b>{$text.CART_PRICE_ALL}</b>
			</div>
		</div>

		<ul class="w-order-product-list">
				...
				{if isset($orderList['CollectionOrderLine'])}
					{foreach $orderList.CollectionOrderLine as $line}
						...
						{if $line->ProductId && is_numeric($line->ProductId)}
							...
							{else}
								{$productImage = $placeholdImage->thumbnail($imageWidth, $imageHeight, true)|solutionPath}
							{/if}
						{/if}
						<li class="w-product-item item-{$line->ProductId}">

								<div class="w-order-product-product">
									{if $productImage}
										<figure class="w-order-product-image">
											<a href="{$link}" title="{$line->ProductTitle}">
												<img src="{$productImage}" class="responsive" alt="{$line->ProductTitle}" itemprop="image"/>
											</a>
										</figure>
									{/if}
									<div class="w-order-product-info">
								...
								<div class="w-order-product-note">
								...
								<div class="w-order-product-amount">
									<span>{$line->Amount} {$line->UnitTitle}</span>
								</div>

								<div class="w-order-product-price">
...
<div class="w-order-summary">
	...
	<div class="m-cart-overview">
		<ul class="m-cart-overview-list">
			{if isset($orderList['CollectionOrderDeliveryLine'])}
				<li>
					<span>{$text.DELIVERY}: {$orderList.CollectionOrderDeliveryLine.0->Title}</span>
			...
			{if isset($orderList['CollectionOrderSubTotalLine'])}
				<li>
					...
					<span>{$orderList.CollectionOrderSubTotalLine.0->Price|formatPriceOrder:$orderId}</span>
				</li>
			{/if}
			{if isset($orderList['CollectionOrderVatTotalLine'])}
				<li>
					...
					<span>{$orderList.CollectionOrderVatTotalLine.0->Price|formatPriceOrder:$orderId}</span>
				</li>
			{/if}
			...
			{if isset($orderList['CollectionOrderPaymentLine']) && $orderList.CollectionOrderPaymentLine.0->Price != 0}
				<li>
					...
					<span><strong>{$orderList.CollectionOrderTotalLine.0->Price|formatPriceOrder:$orderId}</strong></span>
				</li>
			{/if}
		</ul>
	</div>
